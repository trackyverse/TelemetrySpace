...
transformed data {
  ...
  matrix[n_test, n_rec] td;
  for (s in 1:n_test) {
    td[s, ] = to_row_vector(sqrt(square(rec_x - test_x[s]) + square(rec_y - test_y[s])));
  }
}
...
model {
  ...
  for (s in 1:n_test) { // For each test tag
    ...
    vector[n_rec] dist_decay = alpha1 * td[s, ]';
  ...
  for (i in 1:n_ind) {
    for (t in 1:n_time) {
      ...
      vector[n_rec] d = sqrt(square(rec_x - x[i, t]) + square(rec_y - y[i, t]));
      
      // Calculate linear predictor
      vector[n_rec] lp = row(alpha0, t)' - (alpha1 * d);
